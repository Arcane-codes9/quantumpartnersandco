...
export function TradingInterface() {
  ...
  const totalValue = useMemo(() => +user?.balance, [user]);
  ...
  const handleInitTrade = () => {
    ...
    const maturity_amount = ((+amount / 100 * profitPercent) * parseFloat(selectedPlan.duration)) + +amount;
                ...
                {amount && (
                  <div className="space-y-2 p-3 bg-secondary/30 rounded-lg">
                    ...
                    <div className="flex justify-between text-xs text-muted-foreground">
                      ...
                      <span className="font-mono font-medium">
                        ${Number(((+amount / 100 * +selectedPlan?.profit.split('%')[0]) * parseFloat(selectedPlan?.duration)) + +amount).toFixed(2)} USD
                      </span>
                    </div>
